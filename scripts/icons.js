...
// Returns inline SVGs — no external lucide library required
// ============================================================

(function() {
  'use strict';

  /** Inline SVG paths for Lucide icons */
  const PATHS = {
  ...
  // Semantic key → icon name mapping
  const ICON_MAP = {
    'tab-home': 'home',
    'tab-history': 'clock',
    'tab-kb': 'book-open',
  ...
  /**
   * Return an inline SVG string for a semantic key
   * @param {string} key
  ...
  function icon(key, size) {
    ...
    const s = size || 16;
    return `<svg xmlns="http://www.w3.org/2000/svg" width="${s}" height="${s}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="s2ai-icon" aria-hidden="true">${path}</svg>`;
  }
